...
docker run --rm -it --net=docker_hadoop-net -P -e HADOOP_CONF_DATA="core:fs.defaultFS=hdfs://namenode-a:9000 yarn:yarn.resourcemanager.hostname=resourcemanager-a" --hostname spark-on-yarn-debugging --name spark-on-yarn-debugging andy/spark-on-yarn:latest /docker-spark/run.sh -bash

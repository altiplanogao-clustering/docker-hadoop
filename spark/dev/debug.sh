...
docker run --rm -it -p 4040:4040 -P  --net=dockerhadoop_hadoop-net -P -e HADOOP_CONF_DATA="core:fs.defaultFS=hdfs://namenode-a:9000 yarn:yarn.resourcemanager.hostname=resourcemanager-a" --hostname spark-debugging --name spark-debugging andy/spark:latest /docker-spark/run.sh -bash
